...
import com.example.habitbuilder.domain.UserProfile;
import com.example.habitbuilder.repository.UserProfileRepository;
...
/**
 * REST controller for managing {@link com.example.habitbuilder.domain.UserProfile}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<UserProfile> partialUpdateUserProfile(
        ...
        Optional<UserProfile> result = userProfileRepository
            ...
            .map(existingUserProfile -> {
                ...
                if (userProfile.getUserStreak() != null) {
                    existingUserProfile.setUserStreak(userProfile.getUserStreak());
                }
                if (userProfile.getActivityLevel() != null) {
                    existingUserProfile.setActivityLevel(userProfile.getActivityLevel());
                }
                if (userProfile.getDiet() != null) {
                    existingUserProfile.setDiet(userProfile.getDiet());
                }
                if (userProfile.getCooks() != null) {
                    existingUserProfile.setCooks(userProfile.getCooks());
                }
                if (userProfile.getDrives() != null) {
                    existingUserProfile.setDrives(userProfile.getDrives());
                }
